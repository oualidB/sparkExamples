...
object MostPopularSuperhero {
  ...
  //Function to get 10 best SuperHeros
  def bestSuperHeroes (allMarvels : Map[Int, String]) : Map[Int, String] ={
    var topMarvel : Map[Int, String] = Map()
    if(allMarvels.size >= 10){
      topMarvel = allMarvels.take(10)
    }else{
      topMarvel = allMarvels
    }
  return topMarvel
  }
  ...
  def main(args: Array[String]) {
    ...
    val flippedSorted = flipped.sortByKey(false).take(10)
    ...
    println(s"best ten superheroes")
    for (results <- flippedSorted){
      val superHero = namesRdd.lookup(results._2)(0)
      println(s"$superHero")
    }
